...
const RightSide = ({selectedUser}) => {
  return selectedUser && (
    <div className={`bg-[#8185B2]/10 text-white w-full relative overflow-y-scroll custom-scrollbar ${selectedUser?"max-md:hidden":""}`}>
    ...
    <div className='px-5 text-xs'>
      <p>Media</p>
      <div className='mt-2 max-h-[200px] overflow-y-scroll custom-scrollbar grid grid-cols-2 gap-4 opacity-80 '>
        {imagesDummyData.map((url,index)=>{
          return(
            <div key={index} onClick={()=>window.open(url)} className='cursor-pointer rounnded' > 
